SQL Query result after querying grade_event and score tables `SELECT e.event_id,e.date,e.category,sc.student_id,sc.score FROM grade_event e, score sc where e.event_id = sc.event_id;`{{execute}}
